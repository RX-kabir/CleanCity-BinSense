...
namespace SmartWaste.Web.Areas.Operator.Controllers
{
    ...
    public class BinsController : Controller
    {
        ...
        public BinsController(IConfiguration config) => _config = config;

        public async Task<IActionResult> Index(int? zoneId, byte? statusId, string? search)
        {
            var cs = _config.GetConnectionString("DefaultConnection");
            var list = new List<BinListRowVm>();
            ...
            while (await r.ReadAsync())
            {
                list.Add(new BinListRowVm
                {
                    ...
                    LatestTemperature = r["LatestTemperature"] == DBNull.Value ? null : Convert.ToDecimal(r["LatestTemperature"])
                });
            }
